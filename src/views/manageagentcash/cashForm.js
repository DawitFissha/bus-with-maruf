...
import { useTakeFromAgentMutation } from '../../store/bus_api';
const customStyles = {
    content: {
      top: '57%',
      left: '58%',
...
const CashAgentForms = ({data}) => {
  console.log(data)
  const {id}=data
  const dispatch=useDispatch()
  const [takeFromCasher,
  {isLoading,isError,error,isSuccess}]=useTakeFromAgentMutation()
  ...
  const SubmitHandler=()=>{
    ...
    if(amount)
    {
     takeFromCasher({amount:amount,id})
     setAmount('')
    }
    else{
...
const handleSaveStatusClose = (event, reason) => {
    ...
    return (
        ...
        style={customStyles}>
            <Row style={{margin:'0px',padding:'0px',alignItems:'center'}}>
                <Col style={{margin:'0px',padding:'0px',width:'100%'}} xl={12} sm={12} lg={12} md={12}>
                    <Card style={{margin:'0px',padding:'0px'}}>
                        ...
                        {!isLocalError&&isError&&<Alert style={{marginLeft:'-10%',marginTop:'10px',marginBottom:'10px'}} severity="error">{error.data?.message||"connecion error"}</Alert>}  
                           ...
                           <TextField
                            ...
                            <Buttons 
                                ...
                                fullWidth
                                color="primary">
                                {!isLoading?"Collect" :<CircularProgress color='secondary'/>}
                           </Buttons> 
                           </Col> 
                        </Card.Body>
                    </Card>
         ...
         {isSuccess&&<SaveSuccessfull open={saveStatus} handleClose={handleSaveStatusClose} message = 'cash collected' />}
        </React.Fragment>
    );
};

export default CashAgentForms;
