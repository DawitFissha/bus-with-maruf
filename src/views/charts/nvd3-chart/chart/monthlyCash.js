       ...
       useEffect(()=>{
        ...
        if(data)
        {
          ...
          let m=data.getGroupMobileTicketInbr
            ...
            s.map(e=>{
              sales[e.label]={label:days[e.label-1],value:e.totalPrice,
                color:'rgb(62 91 234)'}
              return
            })
            g.map(e=>{
              agent[e.label]={label:days[e.label-1],value:e.totalPrice,
                color:'rgb(189 133 182)'}
              return
            })
            m.map(e=>{
              mobile[e.label]={label:days[e.label-1],value:e.totalPrice,
                color:'rgb(62 191 234'}
              return
            })
            setSales(sales)
